    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex flex-1 items-center justify-center bg-white p-12">
        <form onSubmit={onSubmit} className="w-full max-w-[400px]">
          <h1 className="text-[24px] font-bold tracking-tight text-[#0B2E22]">{t("signInTitle")}</h1>
          <p className="mt-1 text-[13px] text-gray-500">{t("signInSubtitle")}</p>
          ...
          <label className="mt-5 block text-[12.5px] font-semibold text-gray-700">{t("email")}</label>
          <input
            type="email"
            required
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="mt-1.5 h-[42px] w-full rounded-[9px] border border-gray-300 px-3 text-[14px] text-gray-900 outline-none focus:border-[#1F8A5B]"
            placeholder="you@company.com"
          />

          <label className="mt-4 block text-[12.5px] font-semibold text-gray-700">{t("password")}</label>
          <input
            type="password"
            required
            value={password}
          ...
          <p className="mt-5 text-center text-[13px] text-gray-500">
            {t("noAccount")}{" "}
            <Link href="/register" className="font-semibold text-[#1F8A5B] hover:underline">
              {t("createAccount")}
            </Link>
          </p>
        </form>
